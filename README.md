...
- Cấu hình trực tiếp trên giao diện (Stealth, Rotation, Quota, Ollama, ...)
...
Cấu hình qua **Dashboard → Settings → Stealth Mode** hoặc `data/local.yaml`:
...
| `min_request_interval_ms` | Khoảng cách tối thiểu giữa 2 request cùng tài khoản |
| `max_concurrent_per_account` | Đồng thời tối đa (Desktop thật = 1) |
| `per_account_installation_id` | ID riêng tránh liên kết giữa các tài khoản |
| `humanlike_jitter` | Thêm 0.5–2s ngẫu nhiên |
| `hourly_request_warn` | Log cảnh báo vượt ngưỡng |
| `hourly_request_throttle` | Thêm 3–7s delay khi vượt |
| `hourly_request_pause` | Tạm dừng tài khoản khi vượt |
...
| `/admin/stealth-settings` | GET/POST | Cấu hình stealth mode |
